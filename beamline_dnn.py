...
hidden1 = tf.keras.layers.Dense(units=32, kernel_initializer=tf.keras.initializers.HeNormal(), activation='relu')(inputs)
hidden2 = tf.keras.layers.Dense(units=32, kernel_initializer=tf.keras.initializers.HeNormal(), activation='relu')(hidden1)
...
history = dnn.fit(X_train, [Yclf_train, Yreg_train], batch_size=100, epochs=5000, validation_split=0.33, callbacks=[stop])
...
# plots
plot_model(dnn, to_file='beamline_plot.png', show_shapes=True, show_layer_names=True)
